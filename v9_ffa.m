...
audioFiles = dir('*.mp3');  % Change this to '*.wav' if needed
...
for k = 1:length(audioFiles)
    ...
    % Plot the rest of the frequencies (outside 20Hz-20kHz) in purple
    idxLow = f < 20;  % Indices for frequencies below 20 Hz
    idxHigh = f > 20000;  % Indices for frequencies above 20 kHz
    plot(f(idxLow), audioDataFFT(idxLow), 'Color', [0.5, 0, 0.5], 'LineWidth', 1);  % Purple for low freq
    plot(f(idxHigh), audioDataFFT(idxHigh), 'Color', [0.5, 0, 0.5], 'LineWidth', 1);  % Purple for high freq
